...
//        s7.intern();//把World字符串 引用放到 常量池中
        System.out.println(s8 == s7.intern());


        String  s9 = "FF";//常量池中已有，返回已有的引用
        String s10 = new String("FF");//堆中的引用
        System.out.println(s9 == s10.intern());//"FF"已存在常量池中，s10.inter()首先去常量池中查找有，故返回常量池的引用


        String s11 = new String("11");//堆中的引用
        String s12 = new String("11");//堆中的引用
        System.out.println(s11.intern() == s12.intern());//s11.inter()使常量池中有了"11"，故s12.inter()查找时直接返回引用
        ...
        System.out.println(s16 == s15.intern());
        System.out.println(s16 == s15);//但是，s15还是指向堆的引用，s16是常量池中的引用
    }
}
